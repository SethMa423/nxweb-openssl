...
RUN apt-get update \
    && apt-get install -y build-essential autoconf libssl-dev openssl libmagickwand-dev && rm -rf /var/lib/apt/lists/*

COPY . /opt/nxweb-src
WORKDIR /opt/nxweb-src

RUN autoreconf --force --install \
    && ./configure --with-imagemagick --with-openssl --disable-certificates \
    && make && make install \
